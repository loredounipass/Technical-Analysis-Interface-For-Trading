...
import { Send, Bot, User, Trash2, ChevronDown, Cpu, Sparkles, MessageSquare, X, Loader2, Plus, History, Menu } from "lucide-react"
...
export default function AiChat({ symbol, datos, interval = "15m" }) {
  ...
  // Focus input when chat opens or session switches
  useEffect(() => {
  ...
  // Sync current messages to active session in sessions array
  useEffect(() => {
    if (activeSessionId) {
  ...
  // Persistent save sessions to localStorage
  useEffect(() => {
  ...
  const sendMessage = async () => {
    ...
    // Generate context from other sessions to help memory
  ...
  return (
    ...
    >
      ...
      {/* Sidebar */}
      <div 
        className={`absolute top-0 left-0 bottom-0 z-50 w-72 transition-transform duration-300 ease-out flex flex-col border-r border-emerald-900/40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-gray-800">
          ...
          <button 
            onClick={() => setIsSidebarOpen(false)}
            className="p-1 rounded-md text-gray-600 hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-3">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-medium text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/10 transition-all duration-200"
          >
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1 custom-scrollbar">
          {sessions.map(s => (
            <div
              ...
              className={`group flex items-center justify-between px-3 py-3 rounded-xl cursor-pointer transition-all duration-150 ${activeSessionId === s.id ? 'bg-emerald-500/15 border border-emerald-500/20' : 'hover:bg-white/5'}`}
            >
              <div className="flex items-center gap-3 overflow-hidden">
                <MessageSquare className={`w-3.5 h-3.5 flex-shrink-0 ${activeSessionId === s.id ? 'text-emerald-400' : 'text-gray-500'}`} />
                <div className="truncate">
                  <div className={`text-[12px] truncate ${activeSessionId === s.id ? 'text-emerald-300 font-medium' : 'text-gray-300'}`}>
                    {s.messages.length > 0 ? s.messages[0].content.slice(0, 30) : "New Analysis"}
                  </div>
                  <div className="text-[9px] text-gray-500 mt-0.5">{new Date(s.timestamp).toLocaleDateString()}</div>
                </div>
              </div>
              <button 
                onClick={(e) => deleteSession(e, s.id)}
                className="opacity-0 group-hover:opacity-100 p-1 rounded-md text-gray-600 hover:text-red-400 transition-all"
              >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[9px] text-emerald-400/70">{symbol} • Active</p>
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          <textarea
            ...
            disabled={loading}
            className="flex-1 bg-transparent text-[13px] text-white placeholder-gray-600 resize-none outline-none min-h-[20px] max-h-[80px] py-1 overflow-y-auto hide-scrollbar"
